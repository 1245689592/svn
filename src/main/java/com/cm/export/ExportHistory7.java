...
public class ExportHistory7 {
	...
	/**处理2000数据一下的线程*/
	private static ThreadPoolExecutor threadPoolA = (ThreadPoolExecutor)Executors.newFixedThreadPool(1);
	/**处理2000数据以上的线程*/
	private static ThreadPoolExecutor threadPoolB = (ThreadPoolExecutor)Executors.newFixedThreadPool(1);
...
//		exportType = ConfigCommon.getString("export.type", "hex");
		...
		for(final String vin:vins) {
			...
			if(null!=carId) {
				for(int i=0;i<7;i++) {
					final int num = i;
					int count = (int) count(carId,dates[num],dates[num+1]);
					if(count>0&&count<2000) {
						threadPoolA.execute(new Runnable() {
							@Override
							public void run() {
								try {
									exportData(vin,carId,dates[num],dates[num+1]);
								} catch (Exception e) {
									e.printStackTrace();
								}
							}
...
//						});
		...
		timer.schedule(new TimerTask() {
			@Override
			public void run() {
				if(threadPoolA.isTerminated() && threadPoolB.isTerminated()) {
					timer.cancel();
				}else {
					System.out.println("A:"+threadPoolA.getQueue().size());
					System.out.println("B:"+threadPoolB.getQueue().size());
				}
			}
		}, 0l, 10000l);
	}
